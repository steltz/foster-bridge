...
- **Post-landing cache/cost optimizations (2026-08-05):**
  - `backend/src/benchmark/cache-warmer.ts`: `warm()` no-ops entirely when `llm.provider` is `moonshot`. The warmer's premise is Anthropic's (1h ephemeral TTL to beat, cache-write pricing that makes a pre-paid warm worthwhile); Moonshot's implicit cache has no write surcharge and no fixed TTL, so a warm pre-pays exactly the miss the first real request would pay anyway — while its throwaway completion still reasons at full output price (kimi-k3 cannot disable thinking) as untracked spend.
  - `backend/src/moonshot/moonshot.batch-worker.ts`: `drainBatch` gained a phase 0 — ONE global item is primed to completion before the per-group heads run. Distinct (trader, variant) groups of the same benchmark day share the bulk of their rendered prefix (general docs + day bundle), so concurrently-run heads each paid a full miss on that shared portion; one completed item now writes it once. This supersedes the design spec's D7 note that "a single global prime warms only one of many cell prefixes" — both layers now run: global prime, then per-group primes, then fan-out.
  - `backend/src/moonshot/moonshot.envelope.ts`: `prompt_cache_key` is now the sha256 of the SHARED prefix (system message + at most the first two tiers), not the full stable prefix, so every persona/feature variant of a day routes to one cache bucket and cross-variant hits on the shared tiers become possible. The key is only a routing hint — matching stays byte-prefix-based — so the coarsening cannot cause a wrong hit. The worker's `groupByPrefix` envelope hash is therefore no longer 1:1 with `prompt_cache_key` (it remains per-full-envelope, which is what priming granularity needs).
  - `backend/src/moonshot/moonshot.chat.ts`: `createChatWithFallback` latches a rejected (model, schema-hash) pair and skips straight to `json_object` mode on later identical calls — one wasted strict-probe 400 per process instead of one per request. Process-local by design: a restart re-probes once, which is wanted if Moonshot's validator changes.
